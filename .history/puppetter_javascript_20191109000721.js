...
(async () => {
  ...
  await setCookies(page, cookiesPath);
  await page.goto(url, { waitUntil: ['networkidle0', 'load', 'domcontentloaded'] });

  const pageFunction = "Array.from(document.querySelector('#reviewsMedley').querySelectorAll('#cm-cr-dp-review-list > div')).map((item) => item.querySelector('i').innerText.replace(' out of 5 stars', '').replace('.0', '')";

  // async use inside strings is necessary to fix errors if app is packaged with pkg. if pkg not necessary, just use normal evaluate
