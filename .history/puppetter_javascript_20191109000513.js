...
(async () => {
  ...
  const listExample = await page.evaluate(() => Array.from(document.querySelector('#reviewsMedley').querySelectorAll('#cm-cr-dp-review-list > div')).map((item) => item.querySelector('i').innerText.replace(' out of 5 stars', '').replace('.0', '')));
